...
int main(int argc, char **argv) {
    ...
    // A TLS site the data test refused is a thread pointer the guest reads as
    // garbage. Printed only when there is one, so a clean library
    // reads exactly as it always has.
    if (st->tls_refused)
        printf("  TLS sites REFUSED: %u  — garbage thread pointers there\n",
               st->tls_refused);
    printf("  x18 sites: %u    veneered: %u    refused: %u\n",
           st->x18_sites, st->x18_patched, st->x18_refused);
    // CTR_EL0 reads. Printed only when the library has any, so `make check`'s
    // output
