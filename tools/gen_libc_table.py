...
def main():
    ...
    def write(names, dropped):
        with open(out, 'w') as f:
            ...
            if dropped:
                f.write("// Imported by a guest but NOT DECLARED on Darwin, so they cannot be\n"
                        "// forwarded and are left unresolved (the loader refuses them by name):\n")
    ...
    dropped = set()
    for _ in range(8):                     # bounded; each pass drops >= 1 name
        write(fwd, dropped)
        p = subprocess.run(
            ['cc', '-fsyntax-only', '-arch', 'arm64', '-I', os.path.join(ROOT, 'runtime'),
             os.path.join(ROOT, 'runtime/kl_shim.c')],
            capture_output=True, text=True)
        if p.returncode == 0:
            break
        bad = set(re.findall(r"use of undeclared identifier '([A-Za-z_][A-Za-z_0-9]*)'",
                             p.stderr))
        bad &= set(fwd)
        if not bad:
            print("!! kl_shim.c does not compile with the generated table, and the\n"
                  "   errors are not undeclared identifiers — the table is unchanged\n"
                  "   from what produced them:\n" + p.stderr[:2000], file=sys.stderr)
            sys.exit(1)
        dropped |= bad
        fwd = [s for s in fwd if s not in dropped]
        for b in sorted(bad):
            print("  not declared on Darwin, left unresolved: %s" % b, file=sys.stderr)
